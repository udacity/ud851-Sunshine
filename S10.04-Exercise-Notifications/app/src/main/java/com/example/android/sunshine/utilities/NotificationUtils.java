...
//          COMPLETED (3) Create an Intent with the proper URI to start the DetailActivity
            Intent startDetails = new Intent();
            startDetails.setData(WeatherContract.WeatherEntry.CONTENT_URI);

//          COMPLETED (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder stackBuilder = TaskStackBuilder.create(context).addNextIntent(startDetails);
//          COMPLETED (5) Set the content Intent of the NotificationBuilder
            notification_builder.setContentIntent(stackBuilder.getPendingIntent(1001, PendingIntent.FLAG_UPDATE_CURRENT));
...
//          COMPLETED (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(WEATHER_NOTIFICATION_ID, notification_builder.build());
//          COMPLETED (8) Save the time at which the notification occurred using SunshinePreferences
            SunshinePreferences.saveLastNotificationTime(context, SystemClock.currentThreadTimeMillis());
        }
