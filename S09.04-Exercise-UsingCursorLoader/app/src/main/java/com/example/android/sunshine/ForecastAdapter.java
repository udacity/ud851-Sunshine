...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        int dateIndex = MainActivity.DATE_INDEX;
        int descIndex = MainActivity.DESC_INDEX;
        int highIndex = MainActivity.HIGH_INDEX;
        int lowIndex = MainActivity.LOW_INDEX;

        mCursor.moveToPosition(position);
        String weatherForThisDay = "" +
                SunshineDateUtils.getFriendlyDateString(mContext, mCursor.getLong(dateIndex), false) + " - " +
                SunshineWeatherUtils.getStringForWeatherCondition(mContext, mCursor.getInt(descIndex)) + " - " +
                SunshineWeatherUtils.formatHighLows(mContext, mCursor.getInt(highIndex),
                        mCursor.getInt(lowIndex));//mWeatherData[position];
